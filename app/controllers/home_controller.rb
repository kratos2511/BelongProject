class HomeController < ApplicationController
  before_action :authenticate_user!, except: [:index]

  def index
  end
  
  def items
    @items = get_items_for_user
  end
  
  def user_action
    puts params.inspect
    puts current_user.inspect
    item_id = params[:item][:item_id]
    perform_action = params[:item][:perform_action]
    
    action_item = ActionItem.where(user_id: current_user.id, item_id: item_id).first
    action_item = ActionItem.new(user_id: current_user.id, item_id: item_id, read: false, deleted: false) if action_item.blank?
    if(perform_action == "delete")
      action_item.deleted = true
      if action_item.save
        @notice = "Error has occured"
      else
        @notice = "Item deleted."
      end
    else
      action_item.read = true
      if action_item.save
        @notice = "Error has occured"
      else
        @notice = "Item marked read."
      end
    end
    @items = get_items_for_user
    render "items"
  end
  
  def get_items_for_user
    Item.left_joins(:action_items).where("action_items.user_id IS NULL OR (action_items.user_id = ? AND action_items.deleted = ?)", current_user.id, false).select("items.*, action_items.user_id AS user_id, action_items.id AS ai_id,action_items.deleted AS ai_deleted, action_items.read AS ai_read").order("items.posted_on DESC").to_a
  end

end
